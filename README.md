...
<p>
  ...
  <a href="https://github.com/AxiomCodeAI/axiom-code-graph/actions/workflows/nightly.yml"><img alt="Nightly" src="https://github.com/AxiomCodeAI/axiom-code-graph/actions/workflows/nightly.yml/badge.svg"></a>
...
Build: the regression suites and the parser's suites on every merge to `main` (the badge reads "no status" until the workflow has run on `main`). Engines: the prebuilt engine packages on npm; the badge switches to the publish workflow's status once the first publish has run. Nightly: what the pull-request run cannot afford — the engines built for every platform including darwin-arm64, which needs the self-hosted Apple runner and so is skipped on pull requests, and the release packed the way `publish-npm.yml` packs it without uploading.
