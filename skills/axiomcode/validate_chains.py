...
for case in sorted(os.listdir(os.path.join(T, 'cases'))):
    ...
    if not os.path.exists(os.path.join(repo, '.axiomcode', 'out', 'graph.sqlite')):
        ...
        r = subprocess.run(['bash', BUILD, '.'], cwd=repo, capture_output=True, text=True, env=dict(os.environ, AXIOMCODE_LANG=lang, AXIOMCODE_ENGINE=ROOT, AXIOMCODE_SRC='src'))   # the harness parses <case>/src as the client; lib/ is not client code
    ...
    def hash_name(n):   # TS/Java `Owner#member(sig)` / `mod#fn(sig)`; owner lowercase = a TS module
        ...
        ctor = mem in ('<new>', '<init>')
        names = ['<constructor>', '<init>', owner] if ctor else [mem]
        sel = (f"{owner}.{names[0]}" if owner and not owner.islower() else names[0])
        return sel, names, anon
    def dotted(n):      # Python `mod.Class.member` / `mod.f.<locals>.g` / `mod.<module>`
        parts = n.split('.'); mem = parts[-1]
        if mem == '<module>': return f"{parts[0]}.<module>", ['<module>'], False
        owner = parts[-2] if len(parts) > 1 and parts[-2] != '<locals>' and parts[-2][:1].isupper() else None
        names = ['<constructor>', '__init__'] if mem == '__init__' else [mem]
        return (f"{owner}.{mem}" if owner else mem), names, mem.startswith('<')
    for line in open(exp):
        line = line.rstrip('\n')
        m = re.match(r'^(\S+)\t(\S+)\t(.+?)(?: @L(\d+))? -> (.+)$', line)
        if m:
            tier, caller, ln, callee = m.group(1), m.group(3), m.group(4), m.group(5)
            parse = dotted if lang == 'python' else hash_name
    ...
    for tier, csel, ln, tsel, names, anon in edges:
        ...
        if tsel is None:                                                            # an expected blind spot must show as UNRESOLVED, not as an edge
            ...
            else: fwd = any(re.search(lp + rf"\S*(?:{'|'.join(re.escape(n) for n in names)})(?=\s|$)", calls_block(c), re.M) for c in cards(csel))
            if anon: bwd = fwd
            else:
                cpat = re.escape(cm) if not csel.count(':') else r"\S+"
