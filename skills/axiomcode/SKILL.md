...
```
axiomcode build [<repo>] [--lang <l>] [--src <dir>] [--library <root>,…]   parser + engine → .axiomcode/out/graph.sqlite (+ index). Always first.
axiomcode graph [<repo>] [--out <page.html>] [same flags]                  build if needed, then the graph as one page: .axiomcode/graph/graph.html
axiomcode graph export [<repo>] [--out <page.html>]                        the page from an existing graph.sqlite (no engine run, ~1 s)
```
...
- `build` prints resolved / unresolved counts; quote that line. An unresolved call is *unknown, not absent* —
...
`scripts/` holds `axiomcode` (the entry), `axiomcode-build`, `axiomcode-index`, `axiomcode-graph`, `viewer.html`.
