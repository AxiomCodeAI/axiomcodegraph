...
import { extractTopLevelStatements } from '@/parsers/csharp/extractors/cs-member-extractor';
...
import { extractPreprocRegions } from '@/parsers/csharp/extractors/cs-preproc-region-extractor';
import { declarationGroupKey, extractTypes } from '@/parsers/csharp/extractors/cs-type-extractor';
import { childrenOfType, endLine, startColumn, startLine } from '@/parsers/csharp/extractors/cs-node';
...
export class CsFactExtractor {
  ...
  extractFile(options: CsFileExtractionOptions): CsFileFacts {
    ...
    const preprocRegions = extractPreprocRegions({
      root,
      csModuleLinkHash: module.getHash(),
      activeSymbols,
      serviceVersionLinkHash: options.serviceVersionLinkHash,
...
function synthesizedProgramType(
  ...
  if (childrenOfType(root, 'global_statement').length === 0) {
    return undefined;
  }
