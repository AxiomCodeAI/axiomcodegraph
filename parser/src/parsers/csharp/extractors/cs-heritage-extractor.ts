...
export function extractHeritage(
  ...
  // Fork rule 22: `: A, B\n#if X\n , C\n#endif` — a continuation under a
  // `#if` is a `preproc_if` child of the list whose taken branch holds a
  // `base_continuation`; rule 29 is the trailing-comma run, a `base_fragment`.
  // Either way its types are entries like any other, and an untaken branch's
  // are not in the program.
  const entries: Parser.SyntaxNode[] = [];
  for (const child of namedChildren(baseList)) {
    if (child.type !== PREPROC_CHAIN_ROOT) {
      entries.push(child);
      continue;
    }
    const { branches, bodies } = resolvePreprocBranches(child, options.activeSymbols);
    for (const branch of branches) {
      ...
      for (const body of bodies.get(branch.branchIndex) ?? []) {
        if (body.type === 'base_continuation' || body.type === 'base_fragment') {
          entries.push(...namedChildren(body));
        }
      }
    }
  }
