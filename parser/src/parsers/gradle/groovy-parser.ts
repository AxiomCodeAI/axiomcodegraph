...
/**
 * Groovy-specific tree-sitter parser implementation for Gradle build files.
 *
 * Uses tree-sitter-groovy to parse .gradle files into syntax trees.
 * Also handles .gradle.kts conceptually (Kotlin DSL shares structural patterns),
 * though tree-sitter-groovy only parses Groovy syntax.
 */
export class GroovyParser implements LanguageParser {
  readonly language = ProjectLanguage.GROOVY;
  readonly fileExtension = FILE_EXTENSIONS.GRADLE;

  private parser: Parser;

  constructor() {
    this.parser = new Parser();
    this.parser.setLanguage(Groovy);
  }

  /**
   * Parses Groovy/Gradle source code into a syntax tree.
   * Uses callback-based parsing for large files to avoid tree-sitter buffer limits.
   * @param sourceCode Groovy source code as string
    ...
    const parseWithRetry = withRetry(
      ...
      },
      {
        ...
        onRetry: (attempt, error) => {
          console.warn(`[GroovyParser] Parse attempt ${attempt} failed: ${error.message}, retrying...`);
        }
      }
    );
