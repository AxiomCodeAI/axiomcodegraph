...
public class LambdaExamples
{
    ...
    // A `static` lambda that captures nothing.
    public Func<int, int> StaticLambda = static x => x + 1;

    // The EXPLICIT-RETURN-TYPE lambda (C# 10) used to be here and is now in
    // ExplicitReturnTypeLambda.cs, alone. It is outside the vendored grammar
    // (CS-CORPUS-29) and under fork11 its error runs to END OF FILE — so here,
    // at line 63 of 172, it would have swallowed everything below it: capture,
    // argument position, natural type, expression trees. A construct the
    // grammar cannot parse gets its own file so its recovery is MEASURED rather
    // than the surrounding coverage being absorbed into it.
