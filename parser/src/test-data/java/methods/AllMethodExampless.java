package com.inventory.auth.examples2;
...
public class AllMethodExampless {
    ...
    public AllMethodExampless() {
    }

    public AllMethodExampless(String value) {
        this.instanceField = value;
    }

    public <T> AllMethodExampless(T param, Class<T> type) {
        this.instanceField = param != null ? param.toString() : "null";
    }

    public <T, U extends T> AllMethodExampless(T baseParam, U derivedParam, Class<U> derivedType) {
        this.instanceField = derivedParam != null ? derivedParam.toString() : "null";
    }
    ...
    public void receiverParameter(AllMethodExampless this) {
    ...
    public AllMethodExampless recursiveBuilderMethod(int count, String value) {
        if (count > 0) {
    ...
    public boolean equals(Object obj) {
        ...
        AllMethodExampless that = (AllMethodExampless) obj;
    ...
    public <T extends Serializable & Comparable<T>> T intersectionTypeReturn(T input) {
        return input;
    }

    public <T extends Comparable<T> & Cloneable & Serializable> List<T> intersectionTypeInCollection(T value) {
        return Arrays.asList(value);
    }

    public List<Void> getRandomVoidList() {
        return null;
    }

    public <T> List<T> factoryMethodWithDiamond(Class<T> clazz) {
        return new java.util.ArrayList<>();
    }

    public void primitiveArraysInGenericContext(int[][] matrix, byte[]... chunks) {
        System.out.println("Primitive arrays: matrix " + matrix.length + " x " + matrix[0].length + ", chunks " + chunks.length);
    }

    public void mixedPrimitiveAndGenericArrays(int[] primitiveArray, List<Integer>[] genericArrays, double[]... varargsPrimitive) {
        System.out.println("Mixed primitive and generic arrays");
    }

    public <E extends Exception> void throwsGenericException(Supplier<E> exceptionSupplier) throws E {
        throw exceptionSupplier.get();
    }

    public <E extends IOException> void throwsBoundedException(Class<E> exceptionClass) throws E {
        throw null;
    }

    public <T, E extends Exception> T methodWithValueAndException(T value, Supplier<E> exceptionSupplier) throws E {
        if (value == null) throw exceptionSupplier.get();
        return value;
    }

    public <T extends Enum<T>> void explicitEnumBound(Class<T> enumClass, T enumValue) {
        System.out.println("Enum type: " + enumClass.getName() + ", value: " + enumValue);
    }

    public <T extends Enum<T>> T[] enumArrayReturn(Class<T> enumClass) {
        return enumClass.getEnumConstants();
    }

    public <T extends Comparable<T>> T selfReferencingBound(T a, T b) {
        return a.compareTo(b) > 0 ? a : b;
    }

    public <T> T[] genericArrayReturn(Class<T> type, int size) {
        @SuppressWarnings("unchecked")
        T[] array = (T[]) java.lang.reflect.Array.newInstance(type, size);
        return array;
    }

    public <T> T[][] twoDimensionalGenericArrayReturn(Class<T> type, int rows, int cols) {
        @SuppressWarnings("unchecked")
        T[][] array = (T[][]) java.lang.reflect.Array.newInstance(type, rows, cols);
        return array;
    }

    public <T extends Number> T[] boundedGenericArrayReturn(Class<T> type, T... elements) {
        return elements;
    }

    public void arrayDimensionAnnotations(@NotNull String @NotNull [] @NotNull [] arr) {
        System.out.println("Array with type annotations on dimensions: " + arr.length);
    }

    public void annotatedMultiDimensionalArrays(@NotNull Integer @NotNull [] arr, @NotNull List<@NotNull String> @NotNull [] listArray) {
        System.out.println("Multiple annotated arrays");
    }

    public void annotatedWildcard(List<@NotNull ? extends @NotNull Number> list) {
        System.out.println("List with annotated wildcard");
    }

    public void multipleAnnotatedWildcards(
            List<@NotNull ? extends @NotNull Number> upper,
            List<@NotNull ? super @NotNull Integer> lower,
            Map<@NotNull ? extends String, @NotNull ? super Object> mapWildcards) {
        System.out.println("Multiple annotated wildcards");
    }

    public Map<@NotNull String, @NotNull List<@NotNull Integer>> annotatedTypeArgs() {
        Map<@NotNull String, @NotNull List<@NotNull Integer>> map = new HashMap<>();
        return map;
    }

    public @NotNull List<@NotNull Map<@NotNull String, @NotNull Set<@NotNull Integer>>> deeplyAnnotatedTypeArgs() {
        return new java.util.ArrayList<>();
    }

    public <T extends Comparable<? super T> & Iterable<? extends T>> void deepRecursiveBound(T item) {
        System.out.println("Deep recursive bound with Comparable and Iterable");
    }

    public <T extends Comparable<? super T> & Serializable & Cloneable & Iterable<? extends T>> void veryDeepRecursiveBound(T item) {
        System.out.println("Very deep recursive bound with multiple interfaces");
    }

    public <E1 extends IOException, E2 extends RuntimeException> void multiGenericThrows(Class<E1> ex1, Class<E2> ex2) throws E1, E2 {
        throw null;
    }

    public <E1 extends Exception, E2 extends IOException, E3 extends RuntimeException> void tripleGenericThrows() throws E1, E2, E3 {
        throw null;
    }

    public <T, E1 extends Exception, E2 extends RuntimeException> T methodWithMultipleGenericExceptions(T value, Supplier<E1> ex1Supplier, Supplier<E2> ex2Supplier) throws E1, E2 {
        if (value == null) throw ex1Supplier.get();
        return value;
    }

    public static class OuterGeneric<O> {
        private O outerValue;

        public OuterGeneric(O value) {
            this.outerValue = value;
        }

        public <M> Map<O, M> combinedTypeParams(O outer, M method) {
            Map<O, M> map = new HashMap<>();
            map.put(outer, method);
            return map;
        }

        public <M extends O> M methodTypeBoundedByOuter(M methodParam) {
            return methodParam;
        }

        public <M> List<M> methodTypeIndependentOfOuter(M methodParam, O outerParam) {
            List<M> list = new java.util.ArrayList<>();
            list.add(methodParam);
            return list;
        }

        public class InnerGeneric<I> {
            private I innerValue;

            public <M> Map<O, Map<I, M>> tripleTypeParamCombination(@NotNull O outer, I inner, M method) {
                Map<I, M> innerMap = new HashMap<>();
                innerMap.put(inner, method);
                Map<O, Map<I, M>> outerMap = new HashMap<>();
                outerMap.put(outer, innerMap);
                return outerMap;
            }

            public <M extends I> List<M> methodBoundedByInnerType(M methodParam) {
                return Arrays.asList(methodParam);
            }
        }
    }
    ...
    public static class Outer {
        ...
        public void methodWithAnnotatedReceiver(AllMethodExampless.Outer this) {
            System.out.println("Method with receiver parameter: " + this.outerField);
        }
    }

    public <T extends @NotNull Comparable<T>> T annotatedBoundedMethod(@NotNull T value) {
        return value;
    }

    public @NotNull <T> List<@NotNull T> annotatedReturnAndTypeParam(@NotNull Class<@NotNull T> clazz) {
        return new java.util.ArrayList<>();
    }

    public static void main(String[] args) {
        AllMethodExampless example = new AllMethodExampless();
        ...
        AllMethodExampless chained = example.recursiveBuilderMethod(3, "val");
