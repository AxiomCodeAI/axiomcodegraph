...
import { JS_CSV_CHUNK_SIZE } from '../../constants/javascript-constants';
 ...
 *
 * A deliberate near-copy of `ts-relation-writer.ts`. The two front ends are
 * separate by ruling, and the atomicity guarantees below are each the fix for a
 * failure that already happened once — so they are restated here rather than
 * imported, so that a change to TypeScript's writer cannot silently reshape
 * JavaScript's output, and so this file can be read on its own.
 *
...
export class JsRelationWriter {
  ...
  constructor(
    ...
    uniqueSuffix: string,
    /**
     * The relation's header, known UP FRONT.
     *
     * It used to be learned from the first row, so a relation that received
     * no rows produced a 0-byte file — a file with no columns, which is what
     * "no rows" and "the parser never ran" look like when they are
     * indistinguishable, the very thing the touch-every-relation rule exists
     * to prevent. A consumer could not even learn what the relation's columns
     * were. Every registry's header is a constant, so it is passed in.
     */
    knownHeader = ''
  ) {
    this.outputPath = path.join(outputDir, filename);
    this.temporaryPath = `${this.outputPath}.${uniqueSuffix}.partial`;
    this.header = knownHeader;
  }
  ...
  async append(rows: readonly WritableRow[]): Promise<void> {
    ...
    if (this.buffer.length >= JS_CSV_CHUNK_SIZE) {
      await this.flush();
    }
  }
  ...
  async publish(): Promise<void> {
    ...
    this.closed = true;
    if (this.handle === undefined) {
      // An empty relation still gets its file WITH ITS HEADER, so a consumer can
      // tell "no rows" from "the parser never ran" and can still read the
      // columns. A 0-byte file answered neither question.
      await fsp.writeFile(this.outputPath, this.header === '' ? '' : `${this.header}\n`);
      return;
    }
    await this.flush();
