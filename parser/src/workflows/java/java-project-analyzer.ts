    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          const isTestDir = /^tests?$|^__tests__$|^test-|^it$|^integration-tests?$|^e2e$|^spec$/i.test(entry.name);
