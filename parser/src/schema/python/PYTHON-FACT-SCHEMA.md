...
- `constructKind`: `EXEC_COMPLEX_EXPR` (the only measured real gap — `exec tmpl % (a,)`,
  ...
  rejected wholesale, §6.2) \| `ERROR_NODE` \| `MISSING_NODE` \| `SOFT_KEYWORD_MISPARSE`
  (`match`, `case` and `type` are soft keywords: `match = 1` and `match x:` differ only
  by what follows, and a grammar that guesses wrong produces a plausible wrong tree
