 ...
 * what we can parse. See `vendor/tree-sitter-c-sharp/README.md`.
 */
// Bumped from `async1` to `fork6` with the grammar fork (schema v1.6, §4.0.4),
// to `fork7` with the relational-pattern operand rule (CS-CORPUS-23), and to
// `fork8` with the generic object-creation rule (CS-CORPUS-22), and to `fork9`
// with the primary-expression receiver rule (CS-CORPUS-28), and to `fork10` with
// the qualified-creation amendment to rule 8 (CS-CORPUS-27), to `fork11` with
// the generic-call and pointer-indirection rules (CS-CORPUS-26), to `fork12`
// with the async-lambda modifier rule (CS-CORPUS-29), to `fork13` with the
// `#if`-in-a-chain rule, chain and operator forms (CS-CORPUS-30), and to
// `fork14` with the primary-receiver rule for `?.`, the named-tuple-element
// point, the operator-head form and the orphaned else, to `fork15` with the
// parameter and argument fragments (rules 17 and 18), and to `fork16` with
// the arrow and initializer tails (rules 19 and 20), the `;` body and the
// switch section and label under a `#if` (rule 21), and to `fork17` with the
// scanner patch (a verbatim interpolated string ends at one quote), and to
// `fork18` with the base-list continuation, the dangling if and `allows ref
// struct` (rules 22-24), and to `fork19` with a pragma terminator that is a
// newline or end of file and a bare `#error`, and to `fork20` with a
// parenthesized pointer operand, modifiers after `ref struct`'s `ref`, and a
// `#if` around switch-expression arms, `where` clauses and a trailing-comma
// base-list run (rules 25-29), and to `fork21` with the attribute targets as
// reserved identifiers (rule 30), and to `fork22` with modifiers under a `#if`
// and a branch of several attribute lists (rule 31), and to `fork23` with a
// declaration header under a `#if`, `safe`, the null-conditional assignment
// and a constraint continuation (rules 32-35):
// the token is in cs_module's PRIMARY KEY, and two grammars producing the same
// module hash would let a golden blessed under one silently validate the other.
export const CSHARP_GRAMMAR_REGIME = 'ts-cs-0.23.5-abi14-fork23';
