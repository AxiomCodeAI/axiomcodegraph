...
const GATE_CORPUS: Record<string, string> = {
  ...
  // before; 25 of ShareX's 27 delegates and 1 of Polly's 2 were right, because
...
{
    // Fork rule 30: 'property' and 'type' are attribute TARGETS only before a
    // colon. '[property]' is a collection expression whose one element is a
    // local so named — 19 linq-heavy-A sites read it as an attribute list.
    public static object CollectionOfContextualNames(object property, object type)
        => Pair([property], [type]);
    private static object Pair(object a, object b) => a;
...
public class Relational
{
    ...
    public static unsafe int Deref(void* p) => *(int*)p;
...
function grammarGate(): number {
  ...
  try {
    // Constructing the parser runs the gate.
    const parser = new CSharpParser();
    const tree = parser.parse('class C { void M(bool async) { if (async) { } } }');
    if (tree.rootNode.hasError) {
      failures += fail('`if (async)` parses with an ERROR node — the fork is not installed');
    }
  } catch (error) {
  ...
  const relations = new Map(readRelations(outputDir).map((r) => [r.name, r]));
  {
    ...
    if (expressions === undefined || modules === undefined || expressions.header.length === 0) {
      ...
      for (const row of expressions.rows) {
        ...
        const text = lines.get(fileNameByModule.get(row[e('csModuleLinkHash')]!) ?? '');
        if (text === undefined) {
          continue;
        }
        creationsChecked += 1;
        ...
        if (/[A-Za-z0-9_]/.test(last)) {
          endingOnIdentifier.push(`${row[e('startLine')]}:${row[e('startColumn')]}-${row[e('endLine')]}:${row[e('endColumn')]} ends on "${last}"`);
        }
      }
...
async function extensionMembersAreFlattened(): Promise<number> {
  return withTempDir(async (dir) => {
...
`;
    ...
    if (methods === undefined || parameters === undefined || gaps === undefined) {
      return fail('a relation this check reads is missing');
    }
    let bad = 0;
    if (gaps.rows.length !== 0) {
      bad += fail(`a C# 14 extension block produced ${gaps.rows.length} parse-gap rows; the grammar has a rule for it`);
    }
    const iName = methods.header.indexOf('name');
    const iKind = methods.header.indexOf('methodKind');
    const iExt = methods.header.indexOf('isExtension');
    const byName = new Map(methods.rows.map((r) => [r[iName]!, r]));

    // No phantom constructor. This is the row that used to be there.
    if (methods.rows.some((r) => r[iKind] === CsEnums.CsMethodKind.CONSTRUCTOR)) {
      bad += fail(
        '`extension(string source)` produced a CONSTRUCTOR row. A static class has no ' +
          'constructor taking an argument; this is the phantom the rule exists to remove.'
      );
    }
    for (const [name, wantExt, wantParams] of [
      ['Slug', 'true', 1],        // instance member: receiver at position 0
      ['Combine', 'false', 2],    // static member of a block: NO receiver
      ['Zero', 'false', 0],       // type-only receiver: nothing to pass
      ['Classic', 'true', 1],     // the C# 13 form, for comparison
    ] as const) {
      const row = byName.get(name);
      if (row === undefined) {
        bad += fail(`${name} produced no cs_method row; the extension block was not flattened`);
        continue;
      }
      if (row[iKind] !== CsEnums.CsMethodKind.METHOD) {
        bad += fail(`${name} is a ${row[iKind]}, not a METHOD — the block's members were LOCAL_FUNCTIONs before`);
      }
      if (row[iExt] !== wantExt) {
        bad += fail(`${name} has isExtension=${row[iExt]}, expected ${wantExt}`);
      }
      const mine = parameters.rows.filter(
        (p) => p[parameters.header.indexOf('csMethodLinkHash')] === row[methods.header.length - 1]
      );
      if (mine.length !== wantParams) {
        bad += fail(`${name} has ${mine.length} parameters, expected ${wantParams}`);
      }
    }
    // The receiver and the C# 13 `this` parameter must be THE SAME SHAPE.
    const iMode = parameters.header.indexOf('parameterMode');
    const iPos = parameters.header.indexOf('position');
    const modeOf = (method: string): string | undefined => {
      const row = byName.get(method);
      if (row === undefined) {
        return undefined;
      }
      const hash = row[methods.header.length - 1];
      return parameters.rows.find(
        (p) => p[parameters.header.indexOf('csMethodLinkHash')] === hash && p[iPos] === '0'
      )?.[iMode];
    };
    if (modeOf('Slug') !== modeOf('Classic')) {
      bad += fail(
        `the C# 14 receiver is ${modeOf('Slug')} and the C# 13 \`this\` parameter is ` +
          `${modeOf('Classic')}. They lower to the same thing and must read the same.`
      );
    }
    return bad;
  });
...
const RESERVED_ENUM_VALUES: ReadonlyMap<string, string> = new Map([
  [
    ...
    'a node that reports hasError with no ERROR, no MISSING and no zero-width node under ' +
      'it. Its one known trigger — a `#pragma` as the last line of a file with no trailing ' +
      'newline, 36 corpus files — is parsed since fork19 (the terminator is an external ' +
      'token that is a newline or end of file), and a scan of 12,054 files found no other. ' +
      'The extractor`s fallback that emits it stays, as the net under a class of recovery ' +
      'tree-sitter has produced once; a row here would name the next such input.',
  ],
  [
...
// Changed to fork6 … fork11, each in the same commit as the constant, on cs-oracle's ruling
// (schema v1.6 §4.0.4): six grammar rules changed what the parser sees, and a
// module hash that did not change with it would validate the wrong trees.
const FROZEN_GRAMMAR_REGIME = 'ts-cs-0.23.5-abi14-fork23';
...
const NODE_TYPES =
  'src/test/csharp-gates/tree-sitter-c-sharp/src/node-types.json';
...
function parseGapsAreRows(outputDir: string, corpusDir: string): number {
  ...
  // SELF_REPORTING_NODE is RESERVED (see RESERVED_ENUM_VALUES): its one trigger
  // is parsed since fork19, and the enum audit holds it at zero rows.
  for (const bucket of [
    'ERROR_LOCAL',
    'ERROR_TRUNCATING',
    'INSERTED_NODE',
  ...
  // SINCE FORK19 the unterminated file parses too: the pragma's terminator is
  // an external token that is a newline OR end of file. Both files must now
  // produce NO gap; the last-byte pins above still hold, so the day the
  // scanner regresses, GapUnlocatable.cs is the file that says so.
  const triggered = gapsIn('GapUnlocatable.cs');
  if (triggered.length !== 0) {
    failures += fail(
      `a #pragma as the last line with no trailing newline produced ${triggered.length} gap row(s) ` +
        `[${triggered.map((r) => r[c('gapKind')]).join(',')}] — since fork19 the terminator is a newline or end of file`
    );
  }
  if (gapsIn('GapUnlocatableControl.cs').length !== 0) {
    failures += fail(
      'the newline-terminated control produced a gap. The claim is about the terminator, ' +
  ...
  for (const [name, source] of Object.entries(GATE_CORPUS)) {
    if (parser.parse(source).rootNode.hasError) {
      errorFiles.add(name);
    }
  }
  ...
  if (modulesForFiles !== undefined && modulesForFiles.header.length > 0) {
    ...
    expectKind(
      // The `#if`-split accessor list lives in Members.cs.
      'Members.cs',
      'PREPROC_FRAGMENT',
      'A #if the grammar could not nest is the 1.60% irreducible, not the 1.41% that is a ' +
        'grammar gap, and the two need different actions.'
    );
  }
...
function expressionSpine(outputDir: string): number {
  ...
  if (spineCalls('PatRight').length !== 2) {
    failures += fail(
      `${spineCalls('PatRight').length} calls to PatRight, expected 2 — one after ` +
        '`is not null &&` and one after `is null ||`. A constant pattern that takes every ' +
        'binary operator swallows the right operand of the logical expression.'
    );
  }
  const refTarget = spineCalls('RefTarget');
  if (refTarget.length !== 1) {
    failures += fail(
      `${refTarget.length} calls to RefTarget, expected 1 — a ref-returning call on the ` +
        'LEFT of an assignment. Read as a declaration of type RefTarget, the call vanished.'
    );
  } else {
    const invocationRow = byHash.get(refTarget[0]![c('csExpressionLinkHash')]!);
    const parent =
      invocationRow === undefined ? undefined : byHash.get(invocationRow[e('parentExpressionHash')]!);
    if (
      invocationRow === undefined ||
      invocationRow[e('edgeRole')] !== 'ASSIGNMENT_TARGET' ||
      parent === undefined ||
      parent[e('kind')] !== 'ASSIGNMENT'
    ) {
      failures += fail(
        'RefTarget(v) is not the ASSIGNMENT_TARGET child of an ASSIGNMENT row — the ' +
          'structure of `RefTarget(v) = PatValue()` is a call being assigned THROUGH'
      );
    }
  }
  ...
  if (wrapCalls.length < 3) {
    failures += fail(
      `${wrapCalls.length} calls to Wrap, expected at least 3 — one as a NAMED ARGUMENT's ` +
        ...
        'COMMENT sits in front of. Each was lost to a different container and all three ' +
        'to the same shape.'
    );
  }
  // RELATIONAL PATTERNS AS CONDITIONS (CS-CORPUS-23, fork rule 7). In
  // Relational.cs each conditional's CONDITION child is the IS_PATTERN row and
  // the calls in the arms — Store and the throw's constructor — are under the
  // CONDITIONAL, not under the pattern; and a qualified constant keeps its
  // last segment inside the pattern, so the `&&` / `||` row's left operand is
  // the IS_PATTERN itself and Q is its right operand's call.
  {
    ...
    const conditionals = relRows.filter((r) => r[e('kind')] === 'CONDITIONAL');
    if (conditionals.length !== 2) {
      failures += fail(`${conditionals.length} CONDITIONAL row(s) in Relational.cs, expected 2 — one per relational-pattern method`);
    }
    for (const conditional of conditionals) {
      const children = relRows.filter((r) => r[e('parentExpressionHash')] === conditional[ePk]);
      const condition = children.find((r) => r[e('edgeRole')] === 'CONDITION');
      if (condition?.[e('kind')] !== 'IS_PATTERN') {
        failures += fail(`a conditional in Relational.cs has ${condition === undefined ? 'no CONDITION child' : `a ${condition[e('kind')]} condition`} — expected the IS_PATTERN`);
      }
      const arms = children.filter((r) => r[e('edgeRole')] === 'WHEN_TRUE' || r[e('edgeRole')] === 'WHEN_FALSE');
      if (arms.length !== 2) {
        failures += fail(`a conditional in Relational.cs has ${arms.length} arm(s), expected 2 — the pattern swallowed them`);
      }
    }
    const relCalls = callSites.rows.filter((r) => r[c('csModuleLinkHash')] === relHash);
    const count = (name: string): number => relCalls.filter((r) => r[c('calleeName')] === name).length;
    if (count('Store') !== 3 || count('ArgumentOutOfRangeException') !== 2) {
      failures += fail(`Relational.cs: ${count('Store')} call(s) to Store and ${count('ArgumentOutOfRangeException')} throw constructor(s), expected 3 and 2 — the arms of a relational-pattern conditional, plus one in a collection initializer`);
    }
    if (count('Q') !== 2) {
      failures += fail(`Relational.cs: ${count('Q')} call(s) to Q, expected 2 — the right operands after a qualified constant pattern`);
    }
    const logical = relRows.filter((r) => r[e('kind')] === 'BINARY' && (r[e('operatorString')] === '&&' || r[e('operatorString')] === '||'));
    if (logical.length !== 3) {
      failures += fail(`${logical.length} logical BINARY row(s) in Relational.cs, expected 3 — two after qualified constants, one after a positional pattern`);
    }
    for (const row of logical) {
      const left = relRows.find((r) => r[e('parentExpressionHash')] === row[ePk] && r[e('edgeRole')] === 'LEFT_OPERAND');
      if (left?.[e('kind')] !== 'IS_PATTERN') {
        failures += fail(`the ${row[e('operatorString')]} in Relational.cs has ${left === undefined ? 'no left operand' : `a ${left[e('kind')]} left operand`} — expected the IS_PATTERN with Limit.Max inside it, not a member access on it`);
      }
    }
  }

  // A DIRECTIVE BETWEEN OPERANDS (CS-CORPUS-24). In Pragmas.cs every `&&`
  // has a RIGHT operand, the conditional has both arms, and the calls after
  // each directive — IndexOf, Count, Split, EndsWith — exist. Before the
  // fix the pragma was the right operand and the rest of the expression was
  // gone.
  {
    const pragHash = modules.rows.find((r) => (r[m('fileName')] ?? '').endsWith('Pragmas.cs'))?.[modules.header.length - 1] ?? '';
    const pragRows = expressions.rows.filter((r) => r[e('csModuleLinkHash')] === pragHash);
    const ands = pragRows.filter((r) => r[e('kind')] === 'BINARY' && r[e('operatorString')] === '&&');
    if (ands.length !== 2) {
      failures += fail(`${ands.length} && row(s) in Pragmas.cs, expected 2`);
    }
    for (const row of ands) {
      const right = pragRows.find((r) => r[e('parentExpressionHash')] === row[ePk] && r[e('edgeRole')] === 'RIGHT_OPERAND');
      if (right === undefined) {
        failures += fail(`the && on line ${row[e('startLine')]} of Pragmas.cs has no RIGHT operand — a #pragma between the operands took its place`);
      }
    }
    const conditional = pragRows.find((r) => r[e('kind')] === 'CONDITIONAL');
    const arms = conditional === undefined ? [] : pragRows.filter((r) => r[e('parentExpressionHash')] === conditional[ePk] && (r[e('edgeRole')] === 'WHEN_TRUE' || r[e('edgeRole')] === 'WHEN_FALSE'));
    if (arms.length !== 2) {
      failures += fail(`the conditional in Pragmas.cs has ${arms.length} arm(s), expected 2 — a #pragma before the second arm took its place`);
    }
    const pragCalls = new Set(callSites.rows.filter((r) => r[c('csModuleLinkHash')] === pragHash).map((r) => r[c('calleeName')]!));
    for (const name of ['IndexOf', 'Count', 'Split', 'EndsWith']) {
      if (!pragCalls.has(name)) {
        failures += fail(`${name}() after a #pragma inside an expression produced no call site in Pragmas.cs`);
      }
    }
  }

  // A NESTED TYPE OF A GENERIC TYPE (CS-CORPUS-27, fork10). `new
  // Outer<int>.Builder(4)` and the three-segment form are CONSTRUCTOR_CALLs
  // named Builder / Leaf, and nothing named Builder or Leaf is a METHOD_CALL.
  {
    const relHash = modules.rows.find((r) => (r[m('fileName')] ?? '').endsWith('Relational.cs'))?.[modules.header.length - 1] ?? '';
    const relCalls = callSites.rows.filter((r) => r[c('csModuleLinkHash')] === relHash);
    // Builder is created twice (once as the receiver of AddFactory below).
    for (const [name, expected] of [['Builder', 'CONSTRUCTOR_CALL,CONSTRUCTOR_CALL'], ['Leaf', 'CONSTRUCTOR_CALL']] as const) {
      const kinds = relCalls.filter((r) => r[c('calleeName')] === name).map((r) => r[c('callKind')]);
      if (kinds.join(',') !== expected) {
        failures += fail(`\`new Outer<int>…${name}(…)\` is [${kinds.join(',')}], expected [${expected}] — a creation of a nested type of a generic type read as a call on the creation`);
      }
    }
    if (relCalls.some((r) => r[c('calleeName')] === 'Outer')) {
      failures += fail('a call site named Outer exists in Relational.cs — `new Outer<int>` was emitted as its own creation');
    }
  }

  // AWAIT AS AN IDENTIFIER (CS-ORACLE-1). In NotAsync: no AWAIT row; `await +
  // 1` is a BINARY `+` whose left operand is a NAME_REFERENCE `await` linked
  // to the local; `await(2)` is a call named await. In IsAsync: one AWAIT row.
  {
    const awaitHash = modules.rows.find((r) => (r[m('fileName')] ?? '').endsWith('AwaitIdentifier.cs'))?.[modules.header.length - 1] ?? '';
    const notAsync = methods.rows.find((r) => r[mCol('csModuleLinkHash')] === awaitHash && r[mCol('name')] === 'NotAsync')?.[methods.header.length - 1];
    const isAsync = methods.rows.find((r) => r[mCol('csModuleLinkHash')] === awaitHash && r[mCol('name')] === 'IsAsync')?.[methods.header.length - 1];
    const rowsOf = (owner: string | undefined): (readonly string[])[] => expressions.rows.filter((r) => r[e('expressionOwnerHash')] === owner);
    const inNotAsync = rowsOf(notAsync);
    if (inNotAsync.some((r) => r[e('kind')] === 'AWAIT')) {
      failures += fail('NotAsync() has an AWAIT row — `await` is an identifier in a non-async method');
    }
    const plus = inNotAsync.find((r) => r[e('kind')] === 'BINARY' && r[e('operatorString')] === '+' && r[e('rootContext')] === 'VARIABLE_INITIALIZER');
    const left = plus === undefined ? undefined : inNotAsync.find((r) => r[e('parentExpressionHash')] === plus[ePk] && r[e('edgeRole')] === 'LEFT_OPERAND');
    if (left?.[e('kind')] !== 'NAME_REFERENCE' || left[e('potentialQualifiedName')] !== 'await' || left[e('referencedEntityKind')] !== 'LOCAL_VARIABLE') {
      failures += fail(
        `\`await + 1\` in NotAsync() has ${plus === undefined ? 'no BINARY + row' : left === undefined ? 'no left operand' : `a ${left[e('kind')]} ${left[e('potentialQualifiedName')]} (${left[e('referencedEntityKind')]}) left operand`} — expected the NAME_REFERENCE await, a LOCAL_VARIABLE`
      );
    }
    if (!callSites.rows.some((r) => r[c('csModuleLinkHash')] === awaitHash && r[c('calleeName')] === 'await' && r[c('callKind')] === 'FUNCTION_CALL')) {
      failures += fail('`await(2)` in NotAsyncCall() produced no FUNCTION_CALL named await');
    }
    if (rowsOf(isAsync).filter((r) => r[e('kind')] === 'AWAIT').length !== 1) {
      failures += fail(`IsAsync() has ${rowsOf(isAsync).filter((r) => r[e('kind')] === 'AWAIT').length} AWAIT row(s), expected 1 — the async method keeps its await`);
    }

    // ASYNC LAMBDAS (fork12, CS-CORPUS-29). Each lambda in Lambdas() by the
    // local it initialises: five are async, each with exactly one AWAIT row
    // and no NAME_REFERENCE named await; one is not async and has neither.
    // The method itself is NOT async, so `await` inside its lambdas is a
    // keyword only because the LAMBDA says so.
    const lambdasRow = methods.rows.find((r) => r[mCol('csModuleLinkHash')] === awaitHash && r[mCol('name')] === 'Lambdas');
    const lambdasMethod = lambdasRow?.[methods.header.length - 1];
    if (lambdasRow === undefined || lambdasMethod === undefined) {
      failures += fail('Lambdas() produced no method row');
    } else {
      // A lambda row names no enclosing method — its owner is the expression
      // that holds it — so the lambdas of Lambdas() are the LAMBDA rows of its
      // module inside its line range.
      const [from, to] = [Number(lambdasRow[mCol('startLine')]), Number(lambdasRow[mCol('endLine')])];
      const lambdaRows = methods.rows.filter(
        (r) => r[mCol('csModuleLinkHash')] === awaitHash && r[mCol('methodKind')] === 'LAMBDA' &&
          Number(r[mCol('startLine')]) >= from && Number(r[mCol('endLine')]) <= to
      );
      const expected: ReadonlyArray<readonly [number, string, boolean]> = [
        [0, 'inferred', true], [1, 'implicitParameter', true], [2, 'plainReturn', true],
        [3, 'genericReturn', true], [4, 'notAsync', false], [5, 'staticAsync', true],
      ];
      if (lambdaRows.length !== expected.length) {
        failures += fail(`Lambdas() owns ${lambdaRows.length} LAMBDA rows, expected ${expected.length} — an explicit-return-type async lambda that does not parse takes the rest of the method with it`);
      } else {
        const inOrder = [...lambdaRows].sort((a, b) => Number(a[mCol('startLine')]) - Number(b[mCol('startLine')]));
        for (const [i, local, async] of expected) {
          const row = inOrder[i]!;
          if (row[mCol('isAsync')] !== String(async)) {
            failures += fail(`the lambda initialising \`${local}\` has isAsync=${row[mCol('isAsync')]}, expected ${async} — \`async\` read as the lambda's return type rather than its modifier`);
          }
          const body = rowsOf(row[methods.header.length - 1]);
          const awaits = body.filter((r) => r[e('kind')] === 'AWAIT').length;
          const awaitNames = body.filter((r) => r[e('kind')] === 'NAME_REFERENCE' && r[e('potentialQualifiedName')] === 'await').length;
          if (awaits !== (async ? 1 : 0) || awaitNames !== 0) {
            failures += fail(`the lambda initialising \`${local}\` has ${awaits} AWAIT row(s) and ${awaitNames} NAME_REFERENCE(s) named await, expected ${async ? 1 : 0} and 0 — a lambda read as not async walks its await as an identifier`);
          }
        }
        if (Number(inOrder[5]![mCol('isStatic')] === 'true' ? 1 : 0) !== 1) {
          failures += fail('`static async (int id) => …` is not isStatic — two modifiers, both read');
        }
      }
      const viaIdentifier = rowsOf(lambdasMethod).filter((r) => r[e('kind')] === 'NAME_REFERENCE' && r[e('potentialQualifiedName')] === 'async');
      if (viaIdentifier.length !== 2) {
        failures += fail(`\`async + await(async)\` in Lambdas() has ${viaIdentifier.length} NAME_REFERENCE(s) named async, expected 2 — the identifier reading must survive the modifier winning in lambda position`);
      }
    }
  }
  ...
  // arguments, not four. And the pointer indirection of a cast (rule 12)
  // parses: the file has no parse gap.
  {
    const relHash = modules.rows.find((r) => (r[m('fileName')] ?? '').endsWith('Relational.cs'))?.[modules.header.length - 1] ?? '';
    const relCalls = callSites.rows.filter((r) => r[c('csModuleLinkHash')] === relHash);
    ...
    if (gaps !== undefined && gaps.rows.some((r) => r[gaps.header.indexOf('csModuleLinkHash')] === relHash)) {
      failures += fail('Relational.cs has a parse gap — `*(int*)p` (rule 12) or another shape in it does not parse');
    }
  }
  ...
  // Relational.cs checks key on are undisturbed.
  {
    ...
    // FORK RULE 30: `[property]` / `[type]` are COLLECTION expressions, and the
    // call that takes them has two arguments.
    {
      const pair = callSites.rows.find((r) => r[c('csModuleLinkHash')] === relHash && r[c('calleeName')] === 'Pair');
      const collections = relRows.filter((r) => r[e('kind')] === 'COLLECTION_EXPRESSION' && r[e('startLine')] === pair?.[c('startLine')]);
      if (pair?.[c('argumentCount')] !== '2' || collections.length !== 2) {
        failures += fail(`\`Pair([property], [type])\` has ${pair?.[c('argumentCount')] ?? 'no'} argument(s) and ${collections.length} COLLECTION_EXPRESSION row(s) on its line — expected 2 and 2: \`property\` and \`type\` are attribute targets only before a colon`);
      }
    }
...
function attributesAndComments(outputDir: string): number {
  ...
  for (const target of ['NONE', 'ASSEMBLY', 'MODULE', 'RETURN', 'FIELD', 'PROPERTY', 'TYPEVAR']) {
...
async function branchSelection(outputDir: string, corpusDir: string): Promise<number> {
  ...
  // and every receiver row sits INSIDE its parent's span.
  {
    ...
    // BASE-LIST CONTINUATION (rule 22), DANGLING IF (rule 23), ALLOWS (24).
    {
      ...
      const condAssignments = expr.rows.filter((r) => r[ex('csModuleLinkHash')] === moduleHash && r[ex('kind')] === 'ASSIGNMENT' && r[ex('rootContext')] === 'EXPRESSION_STATEMENT' && Number(r[ex('startLine')]) > Number(headerTaken?.[tt('startLine')] ?? 0));
      ...
      const condTargets = condAssignments.map((a) => expr.rows.find((r) => r[ex('parentExpressionHash')] === a[pk] && r[ex('edgeRole')] === 'ASSIGNMENT_TARGET')).filter((t) => t !== undefined && t[ex('isNullConditional')] === 'true');
      if (condTargets.length !== 2) {
        failures += fail(`${condTargets.length} assignment(s) with a null-conditional target in HeaderTaken.Conditional — expected 2: \`other?.Field = 1\` and \`arr?[0] = 2\` are assignments (C# 14)`);
      }
      const fieldsRel = relations.get('all-csharp-fields.csv')!;
      const safeField = fieldsRel.rows.find((r) => r[fieldsRel.header.indexOf('csTypeLinkHash')] === headerHash && r[fieldsRel.header.indexOf('name')] === 'SafeField');
      if (safeField === undefined) {
        failures += fail('`public safe int SafeField;` produced no field row — `safe` is a modifier');
      }
...
async function main(): Promise<number> {
  ...
  try {
    ...
    const CHECKS: Check[] = [
      ...
      {
        name: 'C# 14 extension members are flattened',
        proves:
          'the block`s members belong to the enclosing static class and the receiver is the ' +
          '`this` parameter it lowers to — they were a phantom CONSTRUCTOR and two ' +
          'LOCAL_FUNCTIONs, which is worse than absent',
        run: () => extensionMembersAreFlattened(),
      },
      {
