...
#                      (default ../../../Parser/dist/index.js relative to this repo)
...
echo "▶ solve"
bash "$REPO/src/pipeline/run-souffle.sh" --language javascript \
...
echo "▶ score $PROD"
python3 "$HERE/ground-truth/score.py" "$WORK/ir" "$WORK/out" "$WORK/oracle.tsv" $PROD --dump="$WORK/score-rows.tsv" | tee "$WORK/score.txt"
