...
solve() {
  bash "$ROOT/src/pipeline/run-souffle.sh" --language javascript \
...
for dir in "$HERE"/cases/*/; do
  ...
  if [ $ok = 1 ] && ! python3 "$HERE/tools/coverage_guard.py" "$w/ir" "$w/plain/out" >"$w/coverage.txt" 2>&1; then
    echo "FAIL (silent drop)"; sed 's/^/    /' "$w/coverage.txt" | head -12; ok=0; fi
  if [ $ok = 1 ]; then
    python3 "$HERE/tools/normalize_edges.py" "$w/ir" "$w/plain/out" > "$w/actual.edges" 2>"$w/norm.log" || { echo "FAIL (normalize)"; ok=0; }
  fi
  if [ $ok = 1 ]; then check_golden "$w/actual.edges" "$HERE/expected/$name.edges" "edges" || ok=0; fi
  if [ $ok = 1 ] && [ "$ORACLE" = "1" ]; then
    oracle_check "$w/ir" "$w/plain/out" "$dir/src" "$w" "$HERE/expected/$name.oracle" "$HERE/expected/$name.known-missing" || ok=0
  fi
  if [ $ok = 1 ] && [ "$HAS_LIB" = "1" ]; then
    if ! solve "$w/ir" "$w/libir" "$w/withlib"; then echo "FAIL (solve with library)"; ok=0; fi
    if [ $ok = 1 ] && ! python3 "$HERE/tools/coverage_guard.py" "$w/ir" "$w/withlib/out" >"$w/coverage-lib.txt" 2>&1; then echo "FAIL (silent drop, with library)"; ok=0; fi
    if [ $ok = 1 ]; then
      python3 "$HERE/tools/normalize_edges.py" "$w/ir" "$w/withlib/out" "$w/libir" > "$w/actual.lib.edges" 2>>"$w/norm.log"
