...
#   calls / indirect_call / uses   graphify asserting a call-like relationship
#   method / contains             pure containment — counting them would let it score on
#                                 structure it never claimed was a call
GRAPHIFY_CALL_RELATIONS = {'calls', 'indirect_call'}
GRAPHIFY_GENEROUS = GRAPHIFY_CALL_RELATIONS | {'uses'}


def _graphify_answers(path, relations):
    """graphify's graph.json -> (file, line) -> {(target file, target line)}.
...
def cmd_load_graphify(a):
    ...
    if not by_line:
        ...
        return 2
    rel = GRAPHIFY_GENEROUS if a.generous else GRAPHIFY_CALL_RELATIONS
    answers, rows_in = _graphify_answers(a.graph, rel)
...
def main():
    ...
    p = sub.add_parser('load-graphify'); p.add_argument('db')
    p.add_argument('--project', required=True); p.add_argument('--tool', default='graphify')
    ...
    p.set_defaults(fn=cmd_load_graphify)
