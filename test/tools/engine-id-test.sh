...
f="$(cd "$ROOT" && ls src/java/engine/resolution/*.dl | head -1)"
mutate "$f" "// changed" "editing a rule file ($f)"
mutate "src/java/templates/client-ir.map" "zz_extra_relation	all-zz" "adding a staged relation"
mutate "src/java/souffle/export_manifest.tsv" "zz_pred	zz.csv" "adding an export"
mutate "src/pipeline/engine.conf" 'SOUFFLE_VERSION="9.9"' "bumping the pinned souffle version"
