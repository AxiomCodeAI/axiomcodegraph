...
# Every manifest that repeats the version has to agree with package.json, bump
# or no bump — a plugin manifest left behind reports a version nobody can install.
node .github/scripts/version.mjs check || exit 1

# A bump was made. It has to move forward, and it cannot name a version that was
# already released: npm refuses the publish, and the tag would point at two trees.
if [ "$head_version" != "$base_version" ]; then
  if ! node -e '
    const p = (v) => { const [core, pre] = v.split("-"); return [...core.split(".").map(Number), pre]; };
    const [a, b] = process.argv.slice(1).map(p);
    for (let i = 0; i < 3; i++) if (a[i] !== b[i]) process.exit(b[i] > a[i] ? 0 : 1);
    // same x.y.z: a release outranks its prereleases, prereleases compare as strings
    if (a[3] === undefined) process.exit(1);
    if (b[3] === undefined) process.exit(0);
    process.exit(b[3] > a[3] ? 0 : 1);
  ' "$base_version" "$head_version"; then
    echo "::error::version-gate: $head_version does not come after $base_version"
    exit 1
  fi
  if git rev-parse --verify --quiet "refs/tags/v$head_version" >/dev/null \
     || git ls-remote --exit-code --tags origin "refs/tags/v$head_version" >/dev/null 2>&1; then
    echo "::error::version-gate: v$head_version is already tagged — it has been released. Bump again."
    exit 1
  fi
...
# The base's version has not been released yet: nothing has shipped under it, so
# this change simply joins that release. The gate starts to bite at the tag.
if ! git rev-parse --verify --quiet "refs/tags/v$base_version" >/dev/null \
   && ! git ls-remote --exit-code --tags origin "refs/tags/v$base_version" >/dev/null 2>&1; then
  echo "version $base_version is not released yet (no tag v$base_version); this change ships in it"
  exit 0
fi
