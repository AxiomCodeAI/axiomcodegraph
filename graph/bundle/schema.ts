...
export const VOCAB: readonly VocabSpec[] = [
  ...
  { table: 'entry_points', column: 'reason', value: 'lifecycle', languages: ['java', 'typescript', 'csharp'], meaning: 'Java: `@PostConstruct` / `@PreDestroy` and similar hooks. TypeScript: a hook the container calls by name on a decorated class (`ngOnInit`, `onModuleInit`), which has no call site anywhere. C#: `ExecuteAsync`, `StartAsync` or `StopAsync` on a hosted service.' },
  { table: 'entry_points', column: 'reason', value: 'queue', languages: ['java', 'csharp'], meaning: 'A message-listener method. C#: a broker consumer or a bus message handler.' },
