...
# callee-resolution.dl swap places against their byte order. The include order is part of the
# program text and the program text is hashed, so the compiled-binary cache key becomes a
# function of the user's locale rather than of the rules: a locale change forces a full
# recompile, and two agents on one machine with different locales never share a binary.
# Reproduced on glibc and on MSYS2; invisible on macOS, whose collation matches C either way.
# The sorts below were already forced to C for this reason; the globs were not. See #895.
