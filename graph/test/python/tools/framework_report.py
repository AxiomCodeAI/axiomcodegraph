...
Two families live here, for the same reason. `framework_edge` is the IN-PROCESS hop a
framework makes (a task proxy, a fixture, a route table, a signal, a provider, a hook).
`remote_edge` is the CROSS-PROCESS one — a gRPC client and the servicer that answers it,
joined by the rpc's full method name — with `remote_unserved` / `remote_unsent` carrying
the halves that do not join. Neither is a call edge, so every argument below applies to
both without change.
...
def main():
    ...
    # fact in a repository that holds only one side.
    remote = set()
    p = os.path.join(out, 'remote-edge.csv')
    if os.path.exists(p):
        with open(p, newline='') as fh:
            for r in csv.reader(fh, delimiter='\t'):
                if len(r) >= 5:
