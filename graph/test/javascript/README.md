...
| 49 | a member declared by one link of a CHAINED assignment (#706): `W.api = W.prototype = { … }` declares the literal's methods on `W`, `W.mixin = W.api.mixin = function` and `W.both = W.prototype.both = function` a static method (and a prototype field for the second name), two prototype names for one function, an export beside a prototype member, and one helper installed as a static on two constructors read from another module |
