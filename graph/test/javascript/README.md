...
| 34 | a function handed over as a PROPERTY of an options object: `walk(items, { filter: keep, hooks: { visit } })` into a staged dependency (and unstaged), the same through `opts.filter = keep`, `https.request({ createConnection })`, `new Transform({ transform })`, each a `callback_registered` edge; a project-local callee reading the property gets only its ordinary edge (#643) |
| 35 | a collection's keys (#659): a declared `Map<K, V>` read through `keys()`, `entries()`, the `[key, value]` pair and `forEach`'s second parameter; a `Set<T>`'s `keys()`; object keys written by `set` or seeded by `new Map([[k, v]])` on a plain Map, and a `new Set([...])`'s keys |
| 35 | a default inside a destructuring pattern: `({ mapper = twice } = {})`, a nested `{ opts: { run = inc } = {} }`, an array pattern `[first = twice, second = inc]`, a variable pattern `const { handler = inc } = o`, each called with and without the default overridden, and a pattern whose default is the only value (#673) |
| 37 | `new this.constructor()` inside a base method, `new this()` in a static, and a chained `run()` on each: the receiver's instantiated subclasses, not the declaring class alone (#675, a pin: already resolved by the `this` widening in dispatch.dl) |
| 36 | primitive results: a call on what a project function returns when that is a literal, a concatenation, an arithmetic, comparison, `typeof`, `!` or `-` result, an awaited literal, and a Promise executor's `resolve` / `reject`, each an `ambient_terminal`; `x \|\| name` keeps its project half (#674) |
| 38 | a computed-key write whose value is a read through the same key binding (#676): `object[name] = src[name]` through a variable, inline in a `for..of` over `Object.keys`, and a self-mixin, each resolving one name to its own function (the `Object.keys` loop through #707); an uncorrelated write as the control |
| 39 | a named function expression's own name (#682, #683): shadowed by a `const` of the same name in the body (`Model.aggregate = function aggregate() { const aggregate = new Aggregate(); aggregate.model(this); }`) and by a parameter, and holding the function itself for a recursive call when the expression is a static or prototype assignment on a class-like function |
| 40 | the elements of `Object.keys(o)` / `Object.getOwnPropertyNames(o)` and the key of a `for..in` loop are strings (#685): a string method on one read through `for..of`, `filter`/`forEach`/`reduce`/`map` callbacks, a spread and `keys[0]` is an ambient terminal; a `for..of` still binds the element and a `for..in` over an array binds the index, not the element |
| 41 | an `Object.prototype` member on a project value (#686): `this.hasOwnProperty(n)`, `this.opts.hasOwnProperty(n)`, `Ctor.hasOwnProperty(n)`, `this.constructor.hasOwnProperty(n)`, `this.valueOf()` and `fn.toString()` are ambient terminals; a class that declares `toString` resolves to it, and a base class calling `this.toString()` carries the platform row beside the subclass override |
| 42 | an `instanceof` guard types its subject (#687): a parameter or variable subject through `if`, a ternary, a negated early return, an imported class and a callback parameter over an untyped array; a member subject is left alone; a platform class (`value instanceof Date`) makes the guarded call an ambient terminal; a subject that already resolves keeps its target |
| 43 | a JSDoc generic application of a project class or typedef (#692): `@param {LazySet<string>}` on an imported class, `@param {Ctx<number>}` on a generic shape typedef read through a member chain and through a destructuring pattern; `@returns {Promise<Widget>}` as the unchanged builtin control |
| 44 | a function literal in a position declared with a JSDoc function type takes its parameters' types from it (#691): an argument to `@param {function(Snapshot): boolean}`, a `@type {(w: Widget) => string}` variable, the members of a `@typedef {Object}` with function-typed properties (an arrow and a shorthand method), a `@callback`, and a generic `@typedef {Record<string, (object: O, data: T) => void>}` applied per member with `@template` substitution |
| 45 | `super.apply(dep, source)` and `super.call(x)` inside subclasses whose bases declare `apply` and `call` are the bases' members (#697), not Function.prototype's on `super`; `f.apply(o, args)` and an object's own `apply` are the unchanged controls |
| 46 | `reduce`'s accumulator and the reduce call's value (#699): an array-literal accumulator's `concat` and `push`, a `new Set()` accumulator's `add`, a project-class accumulator whose method is read after the reduce, a chain on the result, `reduce` without an initial value (the accumulator is an element), and `reduceRight` |
| 47 | a `typeof x === 'string'` / `'number'` or `Array.isArray(x)` guard types the subject's references inside the guarded region only (#701): a ternary's true branch, the right side of `&&` through a conjunction, an `if` body; a polymorphic parameter keeps its project target outside the guard and a negated guard types nothing |
| 48 | a staged library packaged as UMD (#710): `module.exports = factory()` inside the wrapper reaches the factory through the wrapper's own parameter, the one library parameter binding admitted, so `umd.format(1)`, a destructured `format(2)` and `new umd.Formatter().render(3)` are `boundary_lib` edges; client-only, the uninstalled package stays `ambiguous_unknown` |
| 49 | a member declared by one link of a CHAINED assignment (#706): `W.api = W.prototype = { … }` declares the literal's methods on `W`, `W.mixin = W.api.mixin = function` and `W.both = W.prototype.both = function` a static method (and a prototype field for the second name), two prototype names for one function, an export beside a prototype member, and one helper installed as a static on two constructors, read from another module, and the const-function constructor as the second link as the recorded gap |
| 50 | a correlated copy whose keys are ENUMERATED from the source (#707): `for (const k in src) dst[k] = src[k]`, `for (const k of Object.keys(src))`, `Object.keys(src).forEach(k => …)`, `Object.getOwnPropertyNames`, a `const names = Object.keys(src)` iterated later, an alias `const ownKeys = Object.keys`, and a project `keys(o)` helper written over `for..in` under an `extend(obj1, obj2)` helper; a key enumerated from another object and a non-copy write under an enumerated key as the controls |
| 51 | a function DECLARATION installed later as a static or object member has the receiver as `this` (#708): `Model.viaDecl = viaDecl`, `obj.onObj = onObj`, `K.viaDeclK = viaDeclK` before its hoisted declaration, a const holding a function expression, a chain onto two constructors, and the two-argument `extend` helper installed on a constructor whose child links to it through `const parent = this` |
| 52 | `this` inside a class static initialization block is the class constructor (#729): an inherited static and an own static called through it, the same call held in a local `const self = this`, beside the by-name and static-method spellings and an instance method's `this` as controls |
| 53 | an extends-clause expression belongs to the callable that lexically encloses the class, not to the module initializer (#730): `class extends this` returned from a static factory and its instance and static members reached through it, beside `extends Base`, a local `const` and a `mix(B)` parameter as controls |
| 55 | a property access whose member is a getter, and an assignment whose member is a setter, are edges to the accessor (#731): a class getter and setter, an inherited getter through `super`, a paired get/set read and written and compound-assigned, an object-literal getter, a destructured read and a getter installed by `Object.defineProperty` on an object and on `this` (#792), and a plain member as the control; the edge carries no call site, as on the TypeScript side |
| 56 | an export names the binding that holds the value, not a same-named function EXPRESSION (#793): a ternary and a `Math.cbrt ||` polyfill keep both operands, beside a function declaration and an `export { named }` clause as the controls |
