...
| 14 | ES re-exports: `export * from`, `export * as ns from`, `export { local as default }`, a mutable `let` export, `await import()` in its three forms, `createRequire`, a side-effect import, an anonymous `export default class extends`, `extends B.Base` through a namespace import; `export { a as b } from` / `export { default } from` / `export { default as x } from` are known-missing (#483, parser) |
| 15 | JSDoc forms: `@this`, `/** @type */ (x)` casts, `@type {T[]}`, `@type {{ w: T }}`, `Map<K,V>`, `Set<T>`, `Promise<T>`, `@typedef`, `import('./types').Name`; a superclass that is a member of a `require()` call is known-missing (PD-JS-8) |
...
| 17 | qualified heritage with a same-named import in the file — the parser's mis-attached import hop is ignored (#479); the dotted text decides |
