...
| 38 | a computed-key write whose value is a read through the same key binding (#676): `object[name] = src[name]` through a variable, inline in a `for..of` over `Object.keys`, and a self-mixin, each resolving one name to its own function; an uncorrelated write as the control |
